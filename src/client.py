...
class SkylandClient:
    ...
    def get_bindings(self):
        ...
        for app in resp['data']['list']:
            ...
            if app_code in SIGN_URL_MAPPING:
    ...
    def run_sign(self, enable_games=None):
        ...
        try:
            ...
            for char in chars:
                ...
                name = char.get('nickName') or char.get('uuid')
                result = None
                ...
                try:
                    ...
                    if result:
                        status = "SUCCESS" if result.get('code') == 0 else "FAIL"
                        msg = result.get('message') or 'OK'
                        if result.get('data') and result['data'].get('awards'):
                            awards = ",".join([f"{a['resource']['name']}x{a['count']}" for a in result['data']['awards']])
                            msg = f"Awards: {awards}"
                        logs.append(f"[{game.upper()}] {name}: {status} - {msg}")
